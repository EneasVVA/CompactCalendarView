...
class CompactCalendarController {
                ...
                if (shouldDrawIndicatorsBelowSelectedDays || (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay && !isCurrentSelectedDay) || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
                    if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
                        ...
                        if (eventsList.size() >= 3) {
                            ...
                            drawNoClickableEvent(canvas, (int) xPosition, (int) yPosition, eventsList);
    ...
    private void drawNoClickableEvent(Canvas canvas, int xPosition, int yPosition, List<Event> events) {
        yPosition -= 35;
        int correctedWidth = widthPerDay - 40;

        for (Event event : events) {
            if (event.getDrawable() != null) {
                Drawable drawable = event.getDrawable();
                drawable.setBounds(xPosition - (correctedWidth / 2), yPosition - (heightPerDay / 2), xPosition + (correctedWidth / 2), yPosition + (heightPerDay / 2));
                event.getDrawable().draw(canvas);
